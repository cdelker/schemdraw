...
class Subroutine(Element):
    ...
    def __init__(self, *args, **kwargs):
        ...
        s = kwargs.get('s', 0.3)
        self.segments.append(Segment([[0, 0], [0, w/2], [h, w/2], [h, -w/2], [0, -w/2], [0, 0]], **kwargs))
        self.segments.append(Segment([[0, w/2-s], [h, w/2-s]], **kwargs))
        self.segments.append(Segment([[0, -w/2+s], [h, -w/2+s]], **kwargs))
        self.params['lblloc'] = 'center'
        self.params['lblofst'] = 0
        self.params['theta'] = -90
...
class Data(Element):
    ...
    def __init__(self, *args, **kwargs):
        ...
        h = kwargs.get('h', 2)
        s = kwargs.get('s', 0.5)
        self.segments.append(Segment([[0, 0], [0, w/2+s/2], [h, w/2-s/2], [h, -w/2-s/2], [0, -w/2+s/2], [0, 0]], **kwargs))
        self.params['lblloc'] = 'center'
        self.params['lblofst'] = 0
        self.params['theta'] = -90
        self.params['drop'] = [h, 0]
...
class Start(Element):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.segments.append(Segment(np.transpose(np.vstack((x, y))), **kwargs))
        self.params['lblloc'] = 'center'
        self.params['lblofst'] = 0
        self.params['theta'] = -90
        self.params['drop'] = [h, 0]
